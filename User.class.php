...
class User extends Chive
{
	 ...
	 * @return bool
	 */
	function addUser($userName, $password, $email, $rid, $hpname, $ip, $access = 1)
	{
		...
		if(!$chk || !$chk[0]) {
			if(is_numeric($access))
			{
				$query = "
					...
					(uname, password, access_level, email, ip, rid, hpname)
					VALUES ($userName, $password, $access, $email, $ip, $rid, $hpname)
					";
				$this->query($query);
